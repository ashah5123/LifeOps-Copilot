...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const raw = typeof window !== "undefined" ? localStorage.getItem("sparkup-state") : null;
        if (!raw) return;
  ...
  useEffect(() => {
    ...
    addToast({
      message: "Gmail connected. Sign in below, then open Inbox to read mail.",
      type: "success",
    });
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      const hint =
        "Could not start Google sign-in. Start the API on port 8000 and set NEXT_PUBLIC_API_URL if needed.";
      setError(hint);
      addToast({ message: hint, type: "error" });
    }
  };

  const handleGitHub = () => {
    addToast({
      message: "GitHub login is not wired yet — use email/password or Google for Gmail.",
      type: "info",
    });
  };
          ...
          <div className="mt-6">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              <motion.button
                ...
                className="flex items-center justify-center gap-2 py-2.5 bg-surface border border-border rounded-xl text-sm font-medium text-text-primary hover:bg-gray-50 transition-colors cursor-pointer"
              >
              ...
              <motion.button
                ...
                className="flex items-center justify-center gap-2 py-2.5 bg-surface border border-border rounded-xl text-sm font-medium text-text-primary hover:bg-gray-50 dark:hover:bg-white/5 transition-colors cursor-pointer"
              >
